...
///CONFIGS///
bool DAMAGEOWNER = false // toggle friendly
...
entity function Turret( entity ent, vector pos, entity owner ) 
{
	...
	if( owner.p.placedTurrets.len() >= MAX_TURRETS )
	{
		foreach(entity part in owner.p.placedTurrets[0].e.turretparts)
		{
			if(IsValid(part)) 
				part.Dissolve( ENTITY_DISSOLVE_CORE, <0, 0, 0>, 5000 )
		}
		owner.p.placedTurrets.remove(0)
	}
	...
	turret_assemble.SetScriptName( "flowstateTurret" )
	...
	owner.p.placedTurrets.append( turret_assemble )
	AddToTrackedEnts_Level( turret_assemble )
	...
	while ( true )
	{
		...
		if(touchingEntsFinal.len() == 0)
		{
			...
			if(turret.e.turretweapon != null)
			{
				...
				if(IsValid(turret.e.turretweapon))
					turret.e.turretweapon.Destroy()
		...
		} else 
		{
			...
			if(IsValid(ent) && ent.IsNPC() && ent.GetTargetName() != "turretweapon" || IsValid(ent) && ent.IsPlayer() )
			{
				turret.e.turretlockedent = ent
				ent.e.islockedent = true
				if(turret.e.onlyonetime == true || isfirsttime )
				{
					if(turret.e.turretweapon == null && IsValid( pc1 ))
					{
				...
				vector targetPos
				if ( ent.IsPlayer() && !ent.IsCrouched() && !ent.IsSliding() || ent.IsNPC() )
				{
					targetPos = ( ent.GetOrigin() + targetOffset ) + ent.GetVelocity() * 0.200 * 0.9
					MakeLookAt( turret, targetPos, ent, fx )
				} else if ( ent.IsPlayer() && ent.IsCrouched() || ent.IsPlayer() && ent.IsSliding() )
				{
...

{
	...
	vector turrentorigin = turret.GetOrigin() + <0.0, 0.0, -25.0>  + (turret.GetForwardVector() * 20)
	
	if( Gamemode() == eGamemodes.fs_spieslegends )
		turrentorigin = turret.GetOrigin() + <0.0, 0.0, -35.0>  + (turret.GetForwardVector() * 20)
	
	dummy.SetParent(turret)
	dummy.SetOrigin(turrentorigin)
