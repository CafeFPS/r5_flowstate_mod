...
global function Sh_GamemodePkknockback_Init
global function OnWeaponPrimaryAttack_SelfPushback
global function OnProjectileCollision_Pk_Pushbacks

global const float RINGFLARE_MAX_RADIUS = 500
global const float RINGFLARE_GROWTH_SPEED = 3.5
global const float RINGFLARE_REDUCESIZE_SPEED = 4

// global const int BOOSTZONE_ALIVE_TIME = 15
global const int BOOSTZONE_MAX_TIMES_GRAB_LIMIT = 5
global const int FIREZONE_MAX_TIMES_GRAB_LIMIT = 3
global const int WAIT_TIME_WHEN_RINGCLOSED = 60

void function Sh_GamemodePkknockback_Init()
{	
    ...
    switch(GetMapName())
    {
		...
		RegisterLocationPK(
            NewLocationSettings(
                "Overflow",
                [
                    NewLocPair(<-323.799377, -16008.7832, 11485.8652>, <0, 24.2251167, 0>),
					NewLocPair(<-323.799377, -16008.7832, 11485.8652>, <0, 24.2251167, 0>)
					// ,
		...
		RegisterLocationPK(
                NewLocationSettings(
                    ...
                    <0, 0, 2500>,$"rui/flowstatelocations/shipment"
                )
            )
...
void function RegisterLocationPK(LocationSettings locationSettings)
{
    #if SERVER
    _RegisterLocationPK(locationSettings)
    #endif
...
var function OnWeaponPrimaryAttack_SelfPushback( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	Assert( player.IsPlayer() )
		...
		EmitSoundOnEntity( player, "JumpPad_LaunchPlayer_3p" )
		...
		player.KnockBack( player.GetVelocity() + player.GetViewForward() * - 800, 0.7 )
		
		thread function() : (player, fx)
		{
			wait 0.6
			foreach(Fx in fx)
				if(IsValid(Fx))
...
void function OnProjectileCollision_Pk_Pushbacks( entity projectile, vector pos, vector normal, entity hitEnt, int hitbox, bool isCritical )
{
	// #if SERVER
		...
		// hitEnt.KnockBack( player.GetViewForward() * 8 * 9, 0.5 ) //*max(1,attacker.p.ampedDamageBonus)
	// #endif
}
