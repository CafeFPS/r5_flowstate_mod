...
void function SND_Lobby()
{
	...
	if(FS_SND.currentRound == 1 && SND_SHOULD_START_A_NEW_MATCH)
	{
		...
		foreach( sPlayer in GetPlayerArray() )
		{
			...
			Remote_CallFunction_UI( sPlayer, "SND_UpdateMaxVotesForTeam", TEAM_IMC, FS_SND.maxvotesallowedforTeamIMC )
			Remote_CallFunction_UI( sPlayer, "SND_UpdateMaxVotesForTeam", TEAM_MILITIA, FS_SND.maxvotesallowedforTeamMILITIA )
		}
...
{	
	if( !IsValid(player) || args.len() != 1 || !FS_SND.selectTeamMenuOpened ) return false
